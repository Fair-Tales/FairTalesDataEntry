...
- **Code reuse over duplication (#129):** reuse is the standard — do not copy-paste a connection, client, or AI-call recipe into a new page/module. When the same logic appears (or is about to appear) in more than one place, extract a single shared helper and call it everywhere. In particular:
  - **S3 filesystem:** construct the app's `s3fs` filesystem ONLY via `utilities.get_s3_filesystem()`. Never write `s3fs.S3FileSystem(...)` inline in a page/module. (The one exception is `scripts/data_cleanup.py`, which runs OUTSIDE Streamlit — no `st.secrets` — and keeps its own construction.)
  - **Anthropic client:** obtain the Claude client ONLY via `utilities.get_anthropic_client()`, which returns `None` when `ANTHROPIC_API_KEY` is absent. Do the `'ANTHROPIC_API_KEY' in st.secrets` guard via `client = get_anthropic_client(); if client is None: <no-API-key UI>`, not by re-checking the secret and constructing `anthropic.Anthropic(...)` inline.
  - **Vision calls:** build image→base64→content-block→call→fence-strip→`json.loads` ONLY via `utilities.vision_json()` (or `vision_text()`/`strip_json_fence()` for the non-JSON cases). Do not hand-roll the request/parse boilerplate. Per the error-handling convention, these helpers let the Anthropic API error propagate and LOG JSON-decode failures rather than silently returning a default.
  - **S3 path constants/helpers:** import `S3_BUCKET`, `NON_BOOK_S3_PREFIXES`, `is_page_image`, `book_folder_name`, `count_folder_pages` from the Streamlit-free `s3_constants` module (shared by both the live app and `scripts/data_cleanup.py`) rather than redefining them.
